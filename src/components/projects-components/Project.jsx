...
const data = [{
    images: ["/projects/project-0.png"], // birden fazla resim eklemek icin: ["/projects/project-0.png", "/projects/project-0-2.png"]
...
{
    images: ["/projects/project-2.png", "/projects/project-2-1.png", "/projects/project-2-2.png", "/projects/project-2-3.png"],
    title: "E-commerce Platform",
    description_en: "A fully functional e-commerce platform with a membership system, product catalog, and a complete shopping flow from cart to order completion.",
    description_tr: "Üyelik sistemi, ürün kataloğu ve sepetten sipariş tamamlamaya kadar uzanan alışveriş akışıyla tam çalışan bir e-ticaret platformu.",
    languages: ["React", "Redux", "Java", "Spring Boot"],
    links: [
        { github: "https://github.com/YureiSh/e-commerce-project", site: "https://e-commerce-project-eae.vercel.app/" }
    ]
},

]

const ProjectCard = ({ item, darkMode, language }) => {
    ...
    const handleLeave = () => {
        setHovered(false);
        setIndex(0); // fare ayrilinca ilk resme don
    };

    return (
        <div className={` ${darkMode ? "bg-[#252128] text-white" : "bg-white"} rounded-md shadow-2xl flex flex-col items-center md:flex-row gap-6 `} >
            <div
                className="w-93.75 h-93.75 flex justify-center relative"
            ...
            >
                {item.images.map((src, i) => (
                    <img
                        key={src}
                        className={`rounded-2xl py-2 px-2 object-cover absolute inset-0 h-full w-full transition-opacity duration-500 ${i === index ? "opacity-100" : "opacity-0"}`}
                        src={src}
                        alt={item.title}
                        width="400"
                        height="400"
